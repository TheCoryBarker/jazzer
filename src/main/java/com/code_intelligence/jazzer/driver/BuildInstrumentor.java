...
public class BuildInstrumentor {
  public static boolean instrumentJars(List<String> jarList)
    ...
    for (String jarpath : jarList) {
      File jar = processSingleJar(jarpath);
  ...
  private static File processSingleJar(String jar) throws IOException {
    // Instrument classes and output new file in output dir
    createInstrumentedClasses(jar);

    // Merge all original class files (that are not newly instrumented) into output dir
